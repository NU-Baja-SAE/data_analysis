...
if engine_col and wheel_col:
    df["cvt_gear_ratio"] = df[engine_col] / df[secondary_col]
...
fig.add_trace(
    go.Scatter(x=df[time_col], y=df[secondary_setpoint], mode="lines", name="sheave_position_setpoint", line=dict(color="blue", dash="dash")),
    row=3, col=1
)
